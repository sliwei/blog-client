      ...
      <div class="item">
        <div class="head">
          <div class="head_img">
            <img
              :src="
                (item.c_user && item.c_user.head_img) ||
                'http://oss.bstu.cn/images/20190401/bfe66y04xh14bcnn.png'
              "
              alt=""
            />
          </div>
        ...
        <div class="main">
          <div class="info">
            <div class="data">
              <p class="name">
                {{ item.c_user && item.c_user.name
                }}<span v-if="item.comment">
                  {{ $t('components.item.reply') }}
                  {{ item.comment.c_user.name }}</span
                >
              </p>
              <p class="time">{{ getTime(item.create_time) }}</p>
            </div>
            <div class="reply">
              <a href="javascript:;" @click="reply(item)">{{
                $t('components.item.reply')
              }}</a>
            </div>
          </div>

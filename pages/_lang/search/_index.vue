  ...
  <div class="index">
    <div class="search">
      <div class="keyword animated faster pulse" v-if="parameter.keyword">
        {{ $t('pages.search.keyword')
        }}<a href="javascript:;">{{ parameter.keyword }}</a>
        {{ $t('pages.search.results') }}
      </div>

      <div
        class="item animated faster pulse"
        v-for="(item, i) in users.list"
        :key="i"
      >
        <div class="time">
...
export default {
  ...
  async asyncData({ app, route, error }) {
    ...
    const { data } = await app.$axios.get(
      `/blog/client/blog/search?keyword=${encodeURIComponent(
        keyword
      )}&pageIndex=${route.params.index || 1}&pageSize=8`
    )
    if (!data.data) {

...
window.tagcloud = (function (win, doc) {
	...
	TagCloud._getConfig = function (config) {
		var defaultConfig = {fontsize: 16, radius: 60, mspeed: "normal", ispeed: "normal", direction: 135, keep: true};
		if (isObject(config)) {
			for (var i in config) {
	...
	TagCloud._getMsSpeed = function (mspeed) {
		var speedMap = {slow: 1.5, normal: 3, fast: 5};
		return speedMap[mspeed] || 3;
	};
	TagCloud._getIsSpeed = function (ispeed) {
		var speedMap = {slow: 10, normal: 25, fast: 50};
	...
	TagCloud.prototype = {
		...
		update: function () {
			...
			for (var j = 0, len = self.items.length; j < len; j++) {
				var rx1 = self.items[j].x, ry1 = self.items[j].y * sc[1] + self.items[j].z * (-sc[0]),
				...
				self.items[j].element.style.left = (self.items[j].x + (self.box.offsetWidth - self.items[j].offsetWidth) / 2) / 4 + "px";
				self.items[j].element.style.top = (self.items[j].y + (self.box.offsetHeight - self.items[j].offsetHeight) / 2) / 4 + "px";
	...
	return function (options) {
		options = options || {};
		var selector = options.selector || '.tagcloud', elements = doc.querySelectorAll(selector), instance = [];
